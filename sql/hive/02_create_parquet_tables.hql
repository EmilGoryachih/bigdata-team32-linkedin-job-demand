...
SELECT
    ...
    CASE WHEN lower(got_summary) IN ('true', 't', '1') THEN true ELSE false END AS got_summary,
    CASE WHEN lower(got_ner) IN ('true', 't', '1') THEN true ELSE false END AS got_ner,
    CASE WHEN lower(is_being_worked) IN ('true', 't', '1') THEN true ELSE false END AS is_being_worked,
    job_title,
    company,
    job_location,
